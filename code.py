...
def setup_watchdog():
    ...
    try:
        # Try different possible watchdog class names
        if hasattr(watchdog, 'WatchDogTimer'):
            wdt = watchdog.WatchDogTimer(timeout=WATCHDOG_TIMEOUT)
        elif hasattr(watchdog, 'WatchDog'):
            wdt = watchdog.WatchDog(timeout=WATCHDOG_TIMEOUT)
        else:
            print("No watchdog timer available")
            return None
